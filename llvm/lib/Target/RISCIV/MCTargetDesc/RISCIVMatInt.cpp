//===- RISCIVMatInt.cpp - Immediate materialisation ------------*- C++ -*--===//
//
...
static void generateInstSeqImpl(int64_t Val,
  ...
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(RISCIVMatInt::Inst(RISCIV::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = RISCIV::ADDI;
      Res.push_back(RISCIVMatInt::Inst(AddiOpc, Lo12));
    }
    return;
  }
  ...
  Res.push_back(RISCIVMatInt::Inst(RISCIV::SLLI, ShiftAmount));
  if (Lo12)
    Res.push_back(RISCIVMatInt::Inst(RISCIV::ADDI, Lo12));
}

namespace llvm {
namespace RISCIVMatInt {
InstSeq generateInstSeq(int64_t Val, const FeatureBitset &ActiveFeatures) {
  ...
  if (Val > 0 && Res.size() > 2) {
    assert(ActiveFeatures[RISCIV::Feature64Bit] &&
    ...
    RISCIVMatInt::InstSeq TmpSeq;
    generateInstSeqImpl(ShiftedVal, ActiveFeatures, TmpSeq);
    TmpSeq.push_back(RISCIVMatInt::Inst(RISCIV::SRLI, LeadingZeros));
