//===----- RISCIVMergeBaseOffset.cpp - Optimise address calculations  -----===//
//
...
#define DEBUG_TYPE "risciv-merge-base-offset"
#define RISCIV_MERGE_BASE_OFFSET_NAME "RISCIV Merge Base Offset"
namespace {

struct RISCIVMergeBaseOffsetOpt : public MachineFunctionPass {
  static char ID;
  ...
  RISCIVMergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return RISCIV_MERGE_BASE_OFFSET_NAME;
  }
...
char RISCIVMergeBaseOffsetOpt::ID = 0;
INITIALIZE_PASS(RISCIVMergeBaseOffsetOpt, DEBUG_TYPE,
                RISCIV_MERGE_BASE_OFFSET_NAME, false, false)
...
bool RISCIVMergeBaseOffsetOpt::detectLuiAddiGlobal(MachineInstr &HiLUI,
                                                  MachineInstr *&LoADDI) {
  if (HiLUI.getOpcode() != RISCIV::LUI ||
      HiLUI.getOperand(1).getTargetFlags() != RISCIVII::MO_HI ||
  ...
  if (LoADDI->getOpcode() != RISCIV::ADDI ||
      LoADDI->getOperand(2).getTargetFlags() != RISCIVII::MO_LO ||
...
// output from LoADDI.
void RISCIVMergeBaseOffsetOpt::foldOffset(MachineInstr &HiLUI,
                                         MachineInstr &LoADDI,
...
bool RISCIVMergeBaseOffsetOpt::matchLargeOffset(MachineInstr &TailAdd,
                                               Register GAReg,
                                               int64_t &Offset) {
  assert((TailAdd.getOpcode() == RISCIV::ADD) && "Expected ADD instruction!");
  ...
  if (OffsetTail.getOpcode() == RISCIV::ADDI) {
    ...
    if (AddiImmOp.getTargetFlags() != RISCIVII::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != RISCIV::LUI ||
        LuiImmOp.getTargetFlags() != RISCIVII::MO_None ||
    ...
    return true;
  } else if (OffsetTail.getOpcode() == RISCIV::LUI) {
...
bool RISCIVMergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &HiLUI,
                                                  MachineInstr &LoADDI) {
...
bool RISCIVMergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
...
FunctionPass *llvm::createRISCIVMergeBaseOffsetOptPass() {
  return new RISCIVMergeBaseOffsetOpt();
}
