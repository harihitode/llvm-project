//===-- RISCIVTargetTransformInfo.cpp - RISC-V specific TTI ---------------===//
//
...
#define DEBUG_TYPE "riscivtti"

InstructionCost RISCIVTTIImpl::getIntImmCost(const APInt &Imm, Type *Ty,
  ...
  return RISCIVMatInt::getIntMatCost(Imm, DL.getTypeSizeInBits(Ty),
                                     getST()->getFeatureBits());
}

InstructionCost RISCIVTTIImpl::getIntImmCostInst(unsigned Opcode, unsigned Idx,
...
InstructionCost
RISCIVTTIImpl::getIntImmCostIntrin(Intrinsic::ID IID, unsigned Idx,
                                   const APInt &Imm, Type *Ty,
                                   TTI::TargetCostKind CostKind) {
...
RISCIVTTIImpl::getPopcntSupport(unsigned TyWidth) {
  assert(isPowerOf2_32(TyWidth) && "Ty width must be power of 2");
  return ST->hasStdExtZbb() ? TTI::PSK_FastHardware : TTI::PSK_Software;
}

bool RISCIVTTIImpl::shouldExpandReduction(const IntrinsicInst *II) const {
...
Optional<unsigned> RISCIVTTIImpl::getMaxVScale() const {
...
InstructionCost RISCIVTTIImpl::getGatherScatterOpCost(
