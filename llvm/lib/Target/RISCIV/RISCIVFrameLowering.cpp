//===-- RISCIVFrameLowering.cpp - RISCIV Frame Information ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the RISCIV implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
static void emitSCSPrologue(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  Register SCSPReg = RISCIVABI::getSCSPReg();
  ...
  const RISCIVInstrInfo *TII = STI.getInstrInfo();
  int64_t SlotSize = STI.getXLen() / 8;
  // Store return address to shadow call stack
  // s[w|d]  ra, 0(s2)
  // addi    s2, s2, [4|8]
  BuildMI(MBB, MI, DL, TII->get(RISCIV::SW))
      .addReg(RAReg)
      .addReg(SCSPReg)
      .addImm(0)
      .setMIFlag(MachineInstr::FrameSetup);
  BuildMI(MBB, MI, DL, TII->get(RISCIV::ADDI))
...
static void emitSCSEpilogue(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  // l[w|d]  ra, -[4|8](s2)
  // addi    s2, s2, -[4|8]
  BuildMI(MBB, MI, DL, TII->get(RISCIV::LW))
...
static int getLibCallID(const MachineFunction &MF,
  ...
  Register MaxReg = RISCIV::NoRegister;
  for (auto &CS : CSI)
    // RISCIVRegisterInfo::hasReservedSpillSlot assigns negative frame indexes to
    // registers which can be saved by libcall.
    if (CS.getFrameIdx() < 0)
      MaxReg = std::max(MaxReg.id(), CS.getReg().id());

  if (MaxReg == RISCIV::NoRegister)
    return -1;

  switch (MaxReg) {
  default:
    llvm_unreachable("Something has gone wrong!");
  case /*s11*/ RISCIV::X27: return 12;
  case /*s10*/ RISCIV::X26: return 11;
  case /*s9*/  RISCIV::X25: return 10;
  case /*s8*/  RISCIV::X24: return 9;
  case /*s7*/  RISCIV::X23: return 8;
  case /*s6*/  RISCIV::X22: return 7;
  case /*s5*/  RISCIV::X21: return 6;
  case /*s4*/  RISCIV::X20: return 5;
  case /*s3*/  RISCIV::X19: return 4;
  case /*s2*/  RISCIV::X18: return 3;
  case /*s1*/  RISCIV::X9:  return 2;
  case /*s0*/  RISCIV::X8:  return 1;
  case /*ra*/  RISCIV::X1:  return 0;
  }
}
...
bool RISCIVFrameLowering::hasFP(const MachineFunction &MF) const {
...
bool RISCIVFrameLowering::hasBP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
void RISCIVFrameLowering::determineFrameLayout(MachineFunction &MF) const {
...
void RISCIVFrameLowering::adjustReg(MachineBasicBlock &MBB,
  ...
  if (isInt<12>(Val)) {
    BuildMI(MBB, MBBI, DL, TII->get(RISCIV::ADDI), DestReg)
        .addReg(SrcReg)
        .addImm(Val)
        .setMIFlag(Flag);
  } else {
    unsigned Opc = RISCIV::ADD;
    bool isSub = Val < 0;
    if (isSub) {
      Val = -Val;
      Opc = RISCIV::SUB;
...
static Register getFPReg(const RISCIVSubtarget &STI) { return RISCIV::X8; }

// Returns the register used to hold the stack pointer.
static Register getSPReg(const RISCIVSubtarget &STI) { return RISCIV::X2; }
...
void RISCIVFrameLowering::emitPrologue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *RVFI = MF.getInfo<RISCIVMachineFunctionInfo>();
  const RISCIVRegisterInfo *RI = STI.getRegisterInfo();
  const RISCIVInstrInfo *TII = STI.getInstrInfo();
  MachineBasicBlock::iterator MBBI = MBB.begin();

  Register FPReg = getFPReg(STI);
  Register SPReg = getSPReg(STI);
  Register BPReg = RISCIVABI::getBPReg();
  ...
  if (hasFP(MF)) {
    ...
    if (RI->hasStackRealignment(MF)) {
      ...
      if (isInt<12>(-(int)MaxAlignment.value())) {
        BuildMI(MBB, MBBI, DL, TII->get(RISCIV::ANDI), SPReg)
            .addReg(SPReg)
        ...
        Register VR =
            MF.getRegInfo().createVirtualRegister(&RISCIV::GPRRegClass);
        BuildMI(MBB, MBBI, DL, TII->get(RISCIV::SRLI), VR)
            .addReg(SPReg)
            .addImm(ShiftAmount)
            .setMIFlag(MachineInstr::FrameSetup);
        BuildMI(MBB, MBBI, DL, TII->get(RISCIV::SLLI), SPReg)
            .addReg(VR)
            .addImm(ShiftAmount)
            .setMIFlag(MachineInstr::FrameSetup);
      }
      ...
      if (hasBP(MF)) {
        // move BP, SP
        BuildMI(MBB, MBBI, DL, TII->get(RISCIV::ADDI), BPReg)
            .addReg(SPReg)
            .addImm(0)
            .setMIFlag(MachineInstr::FrameSetup);
      }
...
void RISCIVFrameLowering::emitEpilogue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {
  const RISCIVRegisterInfo *RI = STI.getRegisterInfo();
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *RVFI = MF.getInfo<RISCIVMachineFunctionInfo>();
...
StackOffset
RISCIVFrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
...
void RISCIVFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                              BitVector &SavedRegs,
  ...
  // pointer.
  if (hasFP(MF)) {
    SavedRegs.set(RISCIV::X1);
    SavedRegs.set(RISCIV::X8);
  }
  // Mark BP as used if function has dedicated base pointer.
  if (hasBP(MF))
    SavedRegs.set(RISCIVABI::getBPReg());
  ...
  if (MF.getFunction().hasFnAttribute("interrupt") && MFI.hasCalls()) {
    ...
    static const MCPhysReg CSRegs[] = { RISCIV::X1,      /* ra */
      RISCIV::X5, RISCIV::X6, RISCIV::X7,                  /* t0-t2 */
      RISCIV::X10, RISCIV::X11,                           /* a0-a1, a2-a7 */
      RISCIV::X12, RISCIV::X13, RISCIV::X14, RISCIV::X15, RISCIV::X16, RISCIV::X17,
      RISCIV::X28, RISCIV::X29, RISCIV::X30, RISCIV::X31, 0 /* t3-t6 */
    };
...
int64_t
RISCIVFrameLowering::assignRVVStackObjectOffsets(MachineFrameInfo &MFI) const {
  int64_t Offset = 0;
...
void RISCIVFrameLowering::processFunctionBeforeFrameFinalized(
    MachineFunction &MF, RegScavenger *RS) const {
  const RISCIVRegisterInfo *RegInfo =
      MF.getSubtarget<RISCIVSubtarget>().getRegisterInfo();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
MachineBasicBlock::iterator RISCIVFrameLowering::eliminateCallFramePseudoInstr(
    MachineFunction &MF, MachineBasicBlock &MBB,
    MachineBasicBlock::iterator MI) const {
  Register SPReg = RISCIV::X2;
  ...
  if (Amount != 0) {
    ...
    if (MI->getOpcode() == RISCIV::ADJCALLSTACKDOWN)
      Amount = -Amount;
...
//   add     sp,sp,-64
uint64_t
RISCIVFrameLowering::getFirstSPAdjustAmount(const MachineFunction &MF) const {
  const auto *RVFI = MF.getInfo<RISCIVMachineFunctionInfo>();
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
bool RISCIVFrameLowering::spillCalleeSavedRegisters(
  ...
  if (SpillLibCall) {
    ...
    BuildMI(MBB, MI, DL, TII.get(RISCIV::PseudoCALLReg), RISCIV::X5)
        .addExternalSymbol(SpillLibCall, RISCIVII::MO_CALL)
        .setMIFlag(MachineInstr::FrameSetup);
...
bool RISCIVFrameLowering::restoreCalleeSavedRegisters(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
  ...
  if (RestoreLibCall) {
    ...
    MachineBasicBlock::iterator NewMI =
        BuildMI(MBB, MI, DL, TII.get(RISCIV::PseudoTAIL))
            .addExternalSymbol(RestoreLibCall, RISCIVII::MO_CALL)
            .setMIFlag(MachineInstr::FrameDestroy);

    // Remove trailing returns, since the terminator is now a tail call to the
    // restore function.
    if (MI != MBB.end() && MI->getOpcode() == RISCIV::PseudoRET) {
...
bool RISCIVFrameLowering::canUseAsPrologue(const MachineBasicBlock &MBB) const {
  ...
  // Inserting a call to a __risciv_save libcall requires the use of the register
  ...
  return !RS.isRegUsed(RISCIV::X5);
}

bool RISCIVFrameLowering::canUseAsEpilogue(const MachineBasicBlock &MBB) const {
  const MachineFunction *MF = MBB.getParent();
  MachineBasicBlock *TmpMBB = const_cast<MachineBasicBlock *>(&MBB);
  const auto *RVFI = MF->getInfo<RISCIVMachineFunctionInfo>();
  ...
  // Using the __risciv_restore libcalls to restore CSRs requires a tail call.
...
bool RISCIVFrameLowering::isSupportedStackID(TargetStackID::Value ID) const {
  switch (ID) {
...
TargetStackID::Value RISCIVFrameLowering::getStackIDForScalableVectors() const {
