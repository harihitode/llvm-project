//===- RISCIVTargetTransformInfo.h - RISC-V specific TTI --------*- C++ -*-===//
//
...
#ifndef LLVM_LIB_TARGET_RISCIV_RISCIVTARGETTRANSFORMINFO_H
#define LLVM_LIB_TARGET_RISCIV_RISCIVTARGETTRANSFORMINFO_H
...
class RISCIVTTIImpl : public BasicTTIImplBase<RISCIVTTIImpl> {
  using BaseT = BasicTTIImplBase<RISCIVTTIImpl>;
  using TTI = TargetTransformInfo;

  friend BaseT;

  const RISCIVSubtarget *ST;
  const RISCIVTargetLowering *TLI;

  const RISCIVSubtarget *getST() const { return ST; }
  const RISCIVTargetLowering *getTLI() const { return TLI; }

public:
  explicit RISCIVTTIImpl(const RISCIVTargetMachine *TM, const Function &F)
...
#endif // LLVM_LIB_TARGET_RISCIV_RISCIVTARGETTRANSFORMINFO_H
